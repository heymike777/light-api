...
import { Chain, Priority } from './types';
import { bs58 } from '@coral-xyz/anchor/dist/cjs/utils/bytes';

export class JitoManager {

    static highTipsAmount = process.env.JITO_TIPS ? +process.env.JITO_TIPS : 0.0001;
    ...
    static async sendBundle(txs: web3.VersionedTransaction[], keypair: web3.Keypair, addTips: boolean = true, recentBlockhash?: string, priority: Priority = Priority.HIGH): Promise<any> {        
        ...
        const txSignatures = txs.map(tx => {
            return bs58.encode(tx.signatures[0]);
        });
        console.log(new Date(), process.env.SERVER_NAME, 'JITO', 'sendBundle', 'txSignatures:', txSignatures);
        ...
        try{
            ...
            if (addTips) {
                if (!recentBlockhash){
                    recentBlockhash = (await SolanaManager.getRecentBlockhash(Chain.SOLANA)).blockhash;
                }
                if (!recentBlockhash) {
            ...
            const bundleResult = await this.searcherClient.sendBundle(bundle);
            try {
                if (bundleResult.ok === false) {
                    console.log(new Date(), process.env.SERVER_NAME, 'JITO', 'sendBundle', 'error:', bundleResult.error);
                }
                else {
                    this.myBundleIds.push(bundleResult.value);
                }
            }
            catch (error) {
